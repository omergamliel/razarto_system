  ...
  useEffect(() => {
    if (isOpen && date && shift) {
      try {
        ...
        const fullShiftEnd = addDays(fullShiftStart, 1);

        let calcStart = fullShiftStart;
        let calcEnd = fullShiftEnd;
        let type = 'full';

        // Check for existing coverages
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto">
            ...
            <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <button type="button" onClick={() => handleTypeChange('partial')} className={`p-4 rounded-xl border-2 transition-all text-center ${coverageType === 'partial' ? 'border-blue-500 bg-blue-50 text-blue-700 font-bold shadow-sm' : 'border-gray-200 text-gray-600 hover:border-blue-200'}`}>
                        <span className="block text-lg">לא</span>
                        <span className="text-xs opacity-70">כיסוי חלקי</span>
                    </button>
                    <button type="button" onClick={() => handleTypeChange('full')} className={`p-4 rounded-xl border-2 transition-all text-center ${coverageType === 'full' ? 'border-blue-500 bg-blue-50 text-blue-700 font-bold shadow-sm' : 'border-gray-200 text-gray-600 hover:border-blue-200'}`}>
